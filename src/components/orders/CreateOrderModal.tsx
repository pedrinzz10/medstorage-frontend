...
export function CreateOrderModal({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  const total = validLines.reduce((acc, l) => {
    ...
    return acc + (product ? product.precoBase * Number(l.quantidade) : 0);
  }, 0);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="neu-card-lg rounded-[24px] w-full max-w-[560px] p-8 max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-[17px] font-extrabold tracking-[-0.4px]">Novo Pedido</h2>
        ...
        <div className="flex flex-col gap-4">
          ...
          {validLines.length > 0 && (
            <div className="neu-inset rounded-[11px] px-4 py-3 flex justify-between items-center">
              <span className="text-[12px]" style={{ color: 'var(--text-muted)' }}>Total estimado</span>
              <span className="text-[17px] font-extrabold" style={{ color: 'var(--accent)' }}>
